...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        alphas = 1.0 - betas
        self.alphas_cumprod = np.cumprod(alphas, axis=0) #(alpha^_)_t
        self.alphas_cumprod_prev = np.append(1.0, self.alphas_cumprod[:-1]) #(alpha^_)_t-1
        self.alphas_cumprod_next = np.append(self.alphas_cumprod[1:], 0.0) #(alpha^_)_t+1
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, step_i=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            print("model_output intensity", model_output.max().item(), model_output.min().item())
            print("target intensity", target.max().item(), target.min().item())
